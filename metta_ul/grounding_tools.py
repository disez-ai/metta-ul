...
def ground_object_atoms(cls, gname=None):
    ...
    if gname:
        ...
        yield escape_dots(prefix), OperationAtom(
            prefix, func_wrapnpop(cls), unwrap=False
        )
    for name, val in inspect.getmembers(cls):
        ...
        if callable(val):
            yield escape_dots(a_name), OperationAtom(
                a_name, class_wrapnpop(name), unwrap=False
            )
        else:
            yield escape_dots(a_name), OperationAtom(
                a_name, prop_wrapnpop(name), unwrap=False
            )
...
def ground_function_atom(func, gname=None):
    ...
    atom = OperationAtom(func_name, func, unwrap=False)
