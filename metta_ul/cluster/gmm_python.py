...
def initialize_gmm(X, K, seed=42):
    ...
    np.random.seed(seed)
    N, D = X.shape
    means = X[np.random.choice(N, K, replace=False)]  # Randomly choose K data points as initial means
    covariances = np.repeat(np.cov(X, rowvar=False)[np.newaxis, :, :], K, axis=0) + 1e-6 * np.eye(D)  # Add small noise for stability
...
def gaussian_pdf(X, means, covariances):
    ...
    mahalanobis = np.einsum('nkd,kde,nke->nk', X_centered, cov_inv, X_centered)  # (N, K)
...
def gmm_em_recursive(X, K, weights, means, covariances, prev_log_likelihood=-np.inf, tol=1e-6, iteration=1, max_iters=100):
    """
    ...
    pdfs = gaussian_pdf(X, means, covariances)  # (N, K)
    weighted_pdfs = pdfs * weights  # (N, K)
    responsibilities = weighted_pdfs / np.sum(weighted_pdfs, axis=1, keepdims=True)  # (N, K)
    ...
    # Update covariances
    X_centered = X[:, np.newaxis, :] - means  # (N, K, D)
    covariances = np.einsum('nk,nkd,nke->kde', responsibilities, X_centered, X_centered) / Nk[:, np.newaxis, np.newaxis]  # (K, D, D)
    ...
    return gmm_em_recursive(X, K, weights, means, covariances, log_likelihood_value, tol, iteration + 1, max_iters)
